...
shrinkAnimation: Animation;
  ...
  setupAnimation() {
    const textSapn = this.fab.querySelector('span');
    const shrink = this.animationCtrl.create('shrink').addElement(this.fab)
    .duration(300).fromTo('width','140px','50px');

    const fade = this.animationCtrl.create('fade')
    .addElement(textSapn).duration(100).fromTo('opacity',1,0)
    .fromTo('width','70px','0px')

    this.shrinkAnimation = this.animationCtrl.create('shrink-animation')
    .duration(400).easing('ease-out').addAnimation([shrink, fade])
  }
